function rho_opt = material_distribution_PTOc_3d(C, TM, q, rho_min, rho_max, design_mask)
% MATERIAL_DISTRIBUTION_PTOC_3D Compute optimal density distribution for compliance-based PTO in 3D
%
%   RHO_OPT = MATERIAL_DISTRIBUTION_PTOC_3D(C, TM, Q, RHO_MIN, RHO_MAX, DESIGN_MASK)
%   computes the optimal density distribution using the Optimality Criteria (OC)
%   method with bisection to enforce the total material constraint for 3D problems.
...
%
% Inputs:
%   C        - Element compliance values (nely x nelx x nelz array)
