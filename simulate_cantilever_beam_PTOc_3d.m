...
% Final visualization
figure('Name', 'Final 3D MBB Beam Optimization Results', 'NumberTitle', 'off', 'Position', [100, 100, 1200, 800]);
...
results_file = sprintf('results/mbb_beam_ptoc_3d_%dx%dx%d_%.2fvol.mat', nelx, nely, nelz, volume_fraction);
...
try
    % Create isosurface for STL export
    if nelz > 1
        [X,Y,Z] = meshgrid(1:nelx, 1:nely, 1:nelz);
        [faces, verts] = isosurface(X, Y, Z, rho_opt, 0.5);
        
        % Scale vertices to actual dimensions (assuming unit elements)
        verts(:,1) = verts(:,1) * dx;
        verts(:,2) = verts(:,2) * dy;
        verts(:,3) = verts(:,3) * dz;
        
        % Export to STL
        stl_file = sprintf('results/mbb_beam_ptoc_3d_%dx%dx%d_%.2fvol.stl', nelx, nely, nelz, volume_fraction);
        stlwrite(stl_file, faces, verts);
        fprintf('STL file exported to: %s\n', stl_file);
    end
catch
    fprintf('STL export skipped (requires stlwrite function)\n');
end

fprintf('\n=== 3D MBB Beam PTOc Optimization Complete ===\n');
