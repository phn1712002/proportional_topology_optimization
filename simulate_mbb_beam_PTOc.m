% SIMULATE_MBB_BEAM_PTOC Run PTOc on MBB beam problem
%
%   This script sets up the MBB beam and runs the compliance minimization PTO algorithm.
%   Uses the modular run_ptoc_iteration function for the main optimization loop.
%
%   Results are saved to 'mbb_beam_PTOc_results.mat' and figures are generated.
...
[fixed_dofs, load_dofs, load_vals, nelx, nely, designer_mask] = mbb_beam_boundary(false);
...
[rho_opt, history, converged, final_iter] = run_ptoc_iteration(...
    ...
    rho_min, rho_max, conv_tol, designer_mask, 'MBB Beam');
...
save('mbb_beam_PTOc_results.mat', 'rho_opt', 'history', 'nelx', 'nely', ...
...
% Save figure
if plot_flag
    saveas(gcf, 'mbb_beam_PTOc_results.png');
end
...
fprintf('Results saved to mbb_beam_PTOc_results.mat and mbb_beam_PTOc_results.png\n');

% Display final design
figure(2);
clf;
imagesc(rho_opt); axis equal tight; colorbar;
title(sprintf('MBB Beam PTOc Final Design (Iteration %d)', final_iter));
axis xy;
xlabel('x'); ylabel('y');
colormap(gray);
drawnow;
